...
from reid.trainers_meannet import CoTrainerAsyMean_5model, CoTrainerAsyMean_6model
...
sys.stdout = Logger("./LOG/MCN_6_mean/D2M/print.txt")
...
def main(args):
    ...
    if args.src_dataset == 'dukemtmc':
        ...
        co5Model_ema = models.create(args.arch, num_classes=632, pretrained=False)
        ...
        co5Model_ema = models.create(args.arch, num_classes=676, pretrained=False)
        ...
        co5Model_ema = models.create(args.arch, num_classes=1041, pretrained=False)
        ...
        co5Model_ema = models.create(args.arch, num_classes=1230, pretrained=False)
    else:
    ...
    if args.resume:
        ...
        co5Model_ema.load_state_dict(checkpoint['state_dict'], strict=False)
    else:
    ...
    co5Model_ema = nn.DataParallel(co5Model_ema).cuda()
    ...
    for param in co4Model_ema.parameters():
        param.detach_()
    for param in co5Model_ema.parameters():
        param.detach_()
    ...
    for iter_n in range(args.iteration):
        ...
        target_features5, _ = extract_features(model_ema, train_in4_loader, print_freq=args.print_freq)
        # synchronization feature order with dataset.train
        ...
        if iter_n == 0:
            # DBSCAN4cluster
            tri_mat5 = np.triu(rerank_dist5, 1)  # tri_mat5.dim=2
            tri_mat5 = tri_mat5[np.nonzero(tri_mat5)]  # tri_mat5.dim=1
            tri_mat5 = np.sort(tri_mat5, axis=None)
            top_num5 = np.round(args.rho * tri_mat5.size).astype(int)
            eps5 = tri_mat5[:top_num5].mean()
            print('eps5 in cluster: {:.3f}'.format(eps5))
        ...
        print('precision5:{}, recall5:{}, fscore5: {}'.format(100 * precision4, 100 * recall5, fscore5))
        ...
        trainer = CoTrainerAsyMean_6model(
            model, coModel, co2Model, co3Model, co4Model, co5Model, model_ema, coModel_ema, co2Model_ema, co3Model_ema, co4Model_ema, co5Model_ema, train_in5_loader, train_out5_loader, train_out4_loader, train_out3_loader, train_out2_loader, unLoader, criterion, optims
        )
