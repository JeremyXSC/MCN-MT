...
from reid.trainers import CoTrainerAsy, CoTrainerAsy3
from reid.trainers_meannet import CoTrainerAsyMean_3model                                                         
...
#sys.stdout = Logger("./logs/dukemar_DBSCAN_3model_mean/print_DBSCAN.txt") 
sys.stdout = Logger("./LOG/MCN_3_mean/D2M/print.txt") 
...
def main(args):
    ...
    if args.src_dataset == 'dukemtmc':
        ...
        coModel_outliers = models.create(args.arch, num_classes=632, pretrained=False)
        coModel_outliers_ema = models.create(args.arch, num_classes=632, pretrained=False)   
        ...
        coModel_outliers = models.create(args.arch, num_classes=676, pretrained=False)
        coModel_outliers_ema = models.create(args.arch, num_classes=676, pretrained=False)
        ...
        coModel_outliers = models.create(args.arch, num_classes=1041, pretrained=False)
        coModel_outliers_ema = models.create(args.arch, num_classes=1041, pretrained=False)
        ...
        coModel_outliers = models.create(args.arch, num_classes=1230, pretrained=False)
        coModel_outliers_ema = models.create(args.arch, num_classes=1230, pretrained=False)
    else:
    ...
    if args.resume:
        ...
        coModel_outliers.load_state_dict(checkpoint['state_dict'], strict=False)
        coModel_outliers_ema.load_state_dict(checkpoint['state_dict'], strict=False)
    else:
    ...
    coModel_outliers = nn.DataParallel(coModel_outliers).cuda()
    coModel_outliers_ema = nn.DataParallel(coModel_outliers_ema).cuda()
    for param in model_ema.parameters():
        param.detach_()  #截断反向传播的梯度流
    for param in coModel_ema.parameters():
        param.detach_()      
    for param in coModel_outliers_ema.parameters():
        param.detach_() 
    ...
    co_outliersOptimizer = torch.optim.Adam(
        coModel_outliers.parameters(), lr = args.lr
    )

    optims = [optimizer, coOptimizer, co_outliersOptimizer]
    ...
    for iter_n in range(args.iteration):
        ...
        # trainer = CoTrainerAsy3(
            # model, coModel, coModel_outliers, train_inliers_loader, train_outliers_loader, unLoader, criterion, optims
        # )
